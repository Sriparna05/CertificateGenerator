...
import { useCertificate } from "@/contexts/CertificateContext"; // Make sure this path is correct
...
export const GenerateSection = ({ onBack }: GenerateSectionProps) => {
  ...
  const startGeneration = async () => {
    // 1. Guard clause to ensure file and template are not null
    if (!file || !selectedTemplate) {
      console.error("Cannot generate: File or template is missing.");
      return;
    }

    // 2. Call generate with ALL required arguments in the correct order
